...
**1. Step files** — Verify all 7 step files exist in `.framework/steps/`:
...
| `step-02b-uix.md` | UIX Spec (Figma mapping) |
...
**4. Skills** — Verify all 12 skill directories exist in `skills/`, each containing a `SKILL.md`:
...
| `skills/uix-creation/` | UIX Spec (Figma mapping) |
...
- Verify that `.cursor/commands/uix-refresh.md` exists (the only path allowed to re-fetch Figma artifacts after step-02b).
