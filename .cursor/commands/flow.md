...
Run the full spec-first feature workflow for a given spec: create/update SPEC, DESIGN, UIX-SPEC (optional), TASKS, implement, and review — step by step with menus to continue, go back, or exit.
...
- **Steps 1-4** (Spec, Design, UIX Spec, Tasks): Artifacts are created on the current branch. No feature branch is created yet. After each approval gate, the step offers to commit the artifact. UIX Spec (step 3) is optional — skip if no Figma. Committing is recommended but not mandatory.
- **Step 5** (Implementation): A feature branch is created via git-worktrees. Code changes happen on this branch.
- **Step 6** (Review): Review happens on the feature branch. On APPROVED [C], finishing-development-branch handles merge/PR/cleanup.
...
- Step files: `.framework/steps/step-00-continue.md` through `step-05-review.md` (includes `step-02b-uix.md`)
- State template: `.framework/templates/workflow-state.template.md`
- Skills: `skills/spec-creation/SKILL.md`, `skills/design-creation/SKILL.md`, `skills/uix-creation/SKILL.md`, `skills/task-creation/SKILL.md`, `skills/implementation/SKILL.md`, `skills/code-review/SKILL.md`
- Templates: `.framework/templates/SPEC.template.md`, `DESIGN.template.md`, `UIX-SPEC.template.md`, `TASKS.template.md`, `REVIEW.template.md`
