...
| `/uix` | Create `specs/XXX-slug/UIX-SPEC.md` (Figma mapping + design context via local `figma-to-code` MCP, saved under `specs/XXX/figma/`) | Spec reference (e.g. 006 or path) |
...
/uix 006
```
→ Maps DESIGN.md segments to Figma files/nodes. If the local `figma-to-code` MCP is connected, fetches `tokens.css` (`get_figma_design_tokens`) and per-node JSX specs (`get_figma_node_spec`), optionally PNGs (`get_figma_frame_with_image`) and SVG/PNG assets (`export_figma_assets`) — all saved under `specs/006-chatbot/figma/`. Creates `specs/006-chatbot/UIX-SPEC.md`. Skip if no Figma designs.
