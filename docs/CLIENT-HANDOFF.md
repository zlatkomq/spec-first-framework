...
| 6 | **Figma → Code video walkthroughs** | [Google Drive folder](https://drive.google.com/drive/folders/1ZkTWf9m8j6lk5mn6FQVqI5HaxhnlZIpW) | Two short screen-recorded walkthroughs showing the Figma → code flow in action: hitting the MCP server from the editor, the cached `figma/` snapshot appearing on disk, and the agent generating implementation from it. Best paired with the Designer Guide for end-to-end orientation. |
...
| **Designer** | **[FIGMA-DESIGNER-GUIDE.md](FIGMA-DESIGNER-GUIDE.md)** — only doc they need | Optional: watch the [Figma → Code videos](https://drive.google.com/drive/folders/1ZkTWf9m8j6lk5mn6FQVqI5HaxhnlZIpW) to see how their Figma file ends up in generated code, and skim [README §Figma and UIX flow](../README.md#figma-and-uix-flow-layout-handoff) for context |
...
| **Engineering manager** | This page, then [Cursor Reporting README](https://github.com/zlatkomq/cursor_reporting) | Understand what telemetry the dashboard surfaces (token cost per developer / model / command) — this is your visibility into how the framework is being used and how much it costs. Read [FAQ.md](FAQ.md) for positioning vs Cursor/Copilot/BMAD/Spec-Kit, reliability guarantees, and privacy/lock-in answers. |
...
Day 1 — Setup (developer + devops)
    ...
    ▸ (Optional) Deploy Cursor Reporting (docker compose up) and run
      `spec-first install-hook` on each developer's machine
