...
**Circular dependency guard**: If a circular dependency is detected during chain walking, INVALIDATE all tasks in the cycle and flag: "Circular dependency detected between [task IDs] — fix in TASKS.md before proceeding."
...
- [ ] Sync TASKS.md checkboxes with task classification (if any tasks were marked `[x]`):
  - UNAFFECTED completed tasks → keep `[x]`
  - INVALIDATED tasks → reset to `[ ]` (even if previously `[x]`)
