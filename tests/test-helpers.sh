#!/usr/bin/env bash
# Helper functions for spec-first-framework skill tests
# Adapted from superpowers/tests/claude-code/test-helpers.sh

# Resolve plugin directory (spec-first-framework root)
...
run_claude() {
    ...
    if _timeout_cmd "$timeout" bash -c "$cmd" > "$output_file" 2>&1; then
        cat "$output_file"
        rm -f "$output_file"
        return 0
    else
