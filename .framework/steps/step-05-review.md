...
driftGlob: '{spec_folder}/figma/drift-T*.md'
...
- If `{uixFile}` exists: read it and list every file under `{figmaDir}/`. Use these as the **definitive design context** for visual / UI findings: `tokens.css` (allowed colors/fonts), `<node-id>.md` (canonical JSX prop tree), optional `<node-id>.png` (visual reference), `assets/`.
- **Never call the `figma-to-code` MCP from this step.** All design context is read from `{figmaDir}/` on disk. If a referenced file is missing, surface as a finding ("Cached Figma snapshot incomplete — recommend `/uix-refresh {spec_id}`"); do NOT call the MCP yourself.
- Read every `{driftGlob}` file. Each unchecked drift item is a candidate **Major** finding (visual/spec mismatch noted during step-04 implementation but not yet resolved). When the implementation has converged on the cached snapshot value, the item should be checked off in the drift file by the implementer; un-checked items are surfaced in REVIEW.md.
- If `figma/` files appear stale relative to UIX-SPEC's mapping (e.g. node IDs in UIX-SPEC have no corresponding `<node-id>.md`), surface as a finding and recommend `/uix-refresh`. **Do not call MCP.**
