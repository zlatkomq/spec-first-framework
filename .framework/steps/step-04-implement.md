...
uixFile: '{spec_folder}/UIX-SPEC.md'
figmaDir: '{spec_folder}/figma'
figmaContextGlob: '{spec_folder}/figma/*.{md,css,png}'
driftArtifactPattern: '{spec_folder}/figma/drift-T<n>.md'
...
Implement all incomplete tasks from TASKS.md. Run a verification gate. Write the implementation summary for cross-spec learning. When **UIX-SPEC** exists and `{figmaDir}/` contains a cached Figma snapshot for this spec, **read those files from disk** as the definitive design context (tokens, per-node specs, optional PNG references). **Never call the `figma-to-code` MCP from this step.**
...
- Load {constitutionRef}, {designFile}, {tasksFile}, and — if present — {uixFile} plus the cached Figma snapshot under `{figmaDir}/` per section 1.
- **No MCP calls from step-04, ever.** All design context is read from `{figmaDir}/` on disk. If a needed file is missing or stale, surface it as a finding (or write a drift entry per § 1) and recommend the user run `/uix-refresh` — do NOT call the MCP yourself.
- **No fidelity loop, no compare-fix iteration.** Implementation is a single pass per task. Observed visual mismatches are recorded **once** in the drift artifact for that task (`{figmaDir}/drift-T<n>.md`) and surfaced to step 05; they do not trigger re-implementation in this step.
...
Do NOT call any `figma-to-code` MCP tool from this step (or from any subagent dispatched from it). All Figma data MUST be read from `{figmaDir}/` on disk.
Do NOT loop or iterate to "achieve pixel parity" with Figma. Single-pass implementation per task; record drift in `{figmaDir}/drift-T<n>.md` and move on.
</HARD-GATE>
...
- If `{uixFile}` exists: read it. List every file under `{figmaDir}/` (pattern `{figmaContextGlob}`) referenced in UIX-SPEC's **Design Context Artifacts** table or present on disk. **Treat these files as the definitive design context** alongside DESIGN.md:
  - `figma/tokens.css` — the **only** allowed source of hex colors and font names for generated code.
  - `figma/<node-id>.md` — canonical JSX prop tree (`x y w h gap padding bg radius border opacity ...`) and flat geometry table; treat numbers as the source of truth for CSS values.
  - `figma/<node-id>.png` — visual reference only; do NOT reverse-engineer pixel values from it (use the `.md` numbers).
  - `figma/assets/*` — exported icons/images.
- If a UI task references Figma but the corresponding `figma/<node-id>.md` is missing: surface as a finding and recommend the user run `/uix-refresh {spec_id}`. Do NOT call the MCP from this step.
- During implementation, if you observe a mismatch between the implementation and a cached `figma/<node-id>.md` value, append a single entry to `{figmaDir}/drift-T<n>.md` (one drift file per task, write-once). The drift file is informational — it does NOT trigger re-fetching or re-implementation.
...
- **Multiple incomplete tasks (default):** Apply {subagentRef}. The subagent skill dispatches a fresh subagent per task with two-stage review (spec compliance, then code quality) after each. All rules from {ruleRef} (TDD mandate, verification iron law, per-task validation gates) apply to each subagent. **Pass** `{uixFile}` (if present) and paths to all files under `{figmaDir}/` into subagent/task context so UI work reads from the same cached design context. **Subagents MUST NOT call the `figma-to-code` MCP** — they read `figma/` files from disk only.
- **Single incomplete task, or user requests direct:** Apply {ruleRef} directly with full context ({tasksFile}, {constitutionRef}, {designFile}, {specFile} for AC reference, plus {uixFile} and `{figmaDir}/` when present as in section 1). **No MCP calls.**
