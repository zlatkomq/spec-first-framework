...
- If `{spec_folder}/UIX-SPEC.md` exists: read it for Figma mapping context. For UI-related tasks, reference the corresponding Figma segment and load all design context files from `{spec_folder}/figma/` listed in UIX-SPEC.md (`tokens.css`, per-node `<node-id>.md`, optional `<node-id>.png`, `assets/*`) as layout reference for implementation. Tokens in `figma/tokens.css` are the **only** allowed source of hex colors and font names — do not invent values. **Also read `.framework/steps/step-02b-uix.md` section 2b** (Figma fidelity loop). Any UI task that has Figma mapping must follow the iterative loop defined there: minimum 2 / maximum 5 compare-fix passes with automatic re-fetch via `get_figma_node_spec` (and optional `get_figma_frame_with_image` for visual parity) and a structured drift file `{spec_folder}/figma/drift-T<n>.md` per pass. Do not treat the first UI build as final.
